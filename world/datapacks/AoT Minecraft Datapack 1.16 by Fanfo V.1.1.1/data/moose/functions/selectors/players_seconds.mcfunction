...
execute as @s[nbt={Inventory:[{Slot:103b,id:"minecraft:carved_pumpkin",tag:{survey_cops:1b}}]}] run effect give @s night_vision 20 0 true

# Kill old
tag @s[tag=impulse2] remove impulse2

# Give and remove gun cart_titan
execute if predicate moose:is_riding_turret run tag @s add gun
execute unless predicate moose:is_riding_turret run tag @s remove gun
