...
execute as @s[tag=!wrap,scores={using_carrot=1..,man_land=0,man_cool=0},predicate=moose:has_gas_trigger,predicate=moose:has_odm] at @s run function moose:mech_gear/throw
...
execute as @s[tag=!wrap,scores={using_carrot=1..,man_land=1..},predicate=moose:has_gas_trigger,predicate=moose:has_odm] at @s run function moose:mech_gear/impulse
execute as @s[tag=impulse2] at @s run execute if block ^ ^ ^1 air run tp @s ^ ^.2 ^.8

# Reset escaneo
scoreboard players remove @s[scores={using_carrot=1..}] using_carrot 1
