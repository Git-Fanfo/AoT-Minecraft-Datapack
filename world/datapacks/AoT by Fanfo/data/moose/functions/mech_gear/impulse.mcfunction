execute as @s[scores={man_land=2..}] run summon area_effect_cloud ^ ^ ^ {Particle:"explosion",NoGravity:1b,ReapplicationDelay:0,Radius:1f,Duration:15,Effects:[{Id:25b,Amplifier:28b,Duration:7}]}

#Falling
execute as @s[scores={man_land=2..}] run effect give @s slow_falling 2 2 true

execute as @e[distance=..6,nbt=!{SelectedItem:{id:"minecraft:carrot_on_a_stick",tag:{maneuver_gear:1b}}},type=!armor_stand,type=!bat, tag=!motion, tag=!target, type=!area_effect_cloud] as @s run scoreboard players add @p near 1

# run scoreboard players add @p near 1

# IF != CERCA DE UNA ENTIDAD
execute as @s[scores={man_land=2..,near=0}] run effect give @s resistance 1 255 true
execute as @s[scores={man_land=2..,near=0}] run summon creeper ^ ^.45 ^-1.7 {NoGravity:1b,Silent:1b,Invulnerable:1b,Fuse:0,Tags:["motion"]}
execute as @s[scores={man_land=2..,near=0}] run summon creeper ^ ^.45 ^-1.7 {NoGravity:1b,Silent:1b,Invulnerable:1b,Fuse:0,Tags:["motion"]}

# IF = CERCA DE UNA ENTIDAD
execute as @s[scores={man_land=2..,near=1..}] run tag @s add impulse2
#reset score 
scoreboard players set @s near 0

#say matar el arnes
execute as @s run function moose:mech_gear/kill
execute as @s run tag @s remove throw
scoreboard players set @p man_land 0

